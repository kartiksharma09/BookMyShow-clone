...
const NavBar = ({ auth: { isAuthenticated }, logout, searchMovieByName,ViewTickets }) => {
  const [movieName, setMovieName] = useState("");

  const onChange = (e) => setMovieName(e.target.value);
  ...
  const authLinks = (
    <div className="form-row">
      <div className="mb-4">
        <Link to="/my-tickets" onClick={ViewTickets} className="btn btn-primary">
          My Ticket
        </Link>
      </div>
...
export default connect(mapStateToProps, { logout, searchMovieByName,ViewTickets })(NavBar);
