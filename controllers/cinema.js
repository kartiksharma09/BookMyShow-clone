...
const cinema = async (req, res) => {
  ...
  try {
    ...
    if (cinemas) {
      return res
        .status(400)
        .json({ msg: "You can't add more then one cinema" });
    }
    cinemas = await Cinema.findOne({ locationOfCinema: locationOfCinema });
    if (cinemas) {
      return res
        .status(400)
        .json({ msg: "Cinema already exist at this place" });
    }
...
const assignMovieToCinema = async (req, res, next) => {
  ...
  const startingTimeToMinutes =
    Number(startingTime[0]) * 60 + Number(startingTime[1]);
  const endingTimeToMinutes =
    Number(endingTime[0]) * 60 + Number(endingTime[1]);

  const currentDuration = endingTimeToMinutes - startingTimeToMinutes;

  const movieDuration = movie.timeDuration.split(":");
  const movieDurationToMinutes =
    Number(movieDuration[0]) * 60 + Number(movieDuration[1]);
  ...
  if (cinema.Movies.length) {
    const movieName = cinema.Movies.filter(
      (movies) =>
        movies.endDate.toISOString().slice(0, 10) >= startDate &&
        (movies.from === from || movies.to === to)
    );
    if (movieName.length) {
      return next({
        status: 400,
