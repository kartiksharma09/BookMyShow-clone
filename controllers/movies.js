...
const searchMovie = async (req, res, next) => {
  ...
  for (let n_cinema = 0; n_cinema < searchCinema.length; n_cinema++) {
    for (
      let n_movies = 0;
      n_movies < searchCinema[n_cinema].Movies.length;
      n_movies++
    ) {
      
      const movies_date = searchCinema[n_cinema].Movies[n_movies].startDate
        .toISOString()
        .slice(0, 10);

      if (
        searchCinema[n_cinema].Movies[n_movies].movieId.toString()
        === searchedMovie._id.toString()

      ) {
        if (currentDate <= movies_date) {
          searchedMovie.releaseDetails.push({
              _id:searchCinema[n_cinema]._id,
            ...
            startDate: searchCinema[n_cinema].Movies[n_movies].startDate
              .toISOString()
              .slice(0, 10),
            endDate: searchCinema[n_cinema].Movies[n_movies].endDate
              .toISOString()
              .slice(0, 10),
          });
        }
